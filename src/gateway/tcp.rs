...
pub async fn handle_tcp_connection(
    ...
    let mut backend_conn = match BackendConnection::connect(backend_addr_str.clone()).await {
        Ok(conn) => {
            log_info!(backend_addr = %backend_addr_str, " Connected to backend");
            conn
        }
        Err(e) => {
            log_error!(backend_addr = %backend_addr_str, error = %e, " Failed to connect to backend");
            let _ = client_socket.write_all(b"Error: Backend unavailable\n").await;
            return Ok(());
        }
    };

    // For TCP connections, we need to handle the backend differently based on type
    match &mut backend_conn {
        BackendConnection::Tcp(backend_stream) => {
            ...
            log_debug!(
                client_addr = %client_addr,
                connection_id = connection_id,
                " Starting bidirectional TCP proxy"
            );
        ...
        BackendConnection::WebSocket(_) => {
            log_error!(
                client_addr = %client_addr,
                connection_id = connection_id,
                " TCP client cannot connect to WebSocket backend"
            );
            let _ = client_socket.write_all(b"Error: Protocol mismatch - TCP client cannot connect to WebSocket backend\n").await;
        }
    }
